...
double manMatrix[4][4][4], invManMatrix[4][4][4];
double reflectivity[4];
double inherentColor[4][3];
double light_in_eye_space[3] ;
double refractivity[4];
...
void findReflectsAndRefracts(int numReflections, double s[3], double e[3], double color[3], int prevObj){
...
if(refractivity[minobj]!=0){
  ...
  if (under > 0) {
    ...
    findReflectsAndRefracts(numReflections-1, nustart, nuend, color, minobj);
...
void drawAllObjects(){
  ...
  for(i=-300; i< 300; i++){
    for (j=-300; j < 300; j++){
      e[0]=i ;
      e[1]=j ;
      findReflectsAndRefracts(3, s, e,  color, 100);
...
int main(){
  ...
  for(int i = -200; i<200; i+=5){
    ...
    makeManMatrix(1, 100, 100, 100, 0, 0, 0, 0, 0, 1400);
    makeManMatrix(2, 100, 100, 100, 0, 0, 0, 150, 150, 1400);
    makeManMatrix(3, 100, 100, 100, 0, 0, 0, -150, -150, 1400);

    inherentColor[0][0]=.4; inherentColor[0][1]=.4; inherentColor[0][2]=.4;
    inherentColor[1][0]=0; inherentColor[1][1]=1; inherentColor[2][2]=1;
    inherentColor[2][0]=1; inherentColor[2][1]=0; inherentColor[2][2]=1;
    inherentColor[3][0]=1; inherentColor[3][1]=1; inherentColor[3][2]=0;

    reflectivity[0]=0; reflectivity[1]=.5; reflectivity[2]=.5; reflectivity[3]=.5;
    refractivity[0]=.8; refractivity[1]=0; refractivity[2]=0; refractivity[3]=0;
    refractiveIndex[0]=2.4; refractiveIndex[1]=1.4; refractiveIndex[2]=1.4; refractiveIndex[3]=1.4;
